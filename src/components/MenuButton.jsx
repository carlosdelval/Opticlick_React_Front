...
const MenuButton = ({ text, classes, action, icon, iconRight = false }) => {
  return (
    <button
      onClick={action}
      className={`${classes} text-babypowder dark:border-gray-300 bg-chryslerblue dark:bg-vistablue dark:text-babypowder border border-black relative inline-flex items-center justify-center h-12 px-6 overflow-hidden font-medium rounded-md group`}
    >
      {!iconRight && (
